...
def import_yunlifang():
    ...
    # 第2行是设备名称行，每4列一个设备 (设备名、槽位号、板卡类型、端口信息)
    device_row = 2
    col = 2  # 从B列开始
    ...
    for device_info in huawei_devices:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (site_id, device_info['name'], device_info['code'], 'HUAWEI', device_model, 'active', datetime.now()))
        device_id = cursor.lastrowid
        devices_imported += 1

        # 读取该设备的所有槽位 (第2-9行，对应槽位1-8)
        for slot_num in range(1, 9):
            slot_row = device_row + slot_num - 1  # 第2行是槽位1

            slot_num_cell = ws.cell(row=slot_row, column=device_info['col'] + 1)
            card_type_cell = ws.cell(row=slot_row, column=device_info['col'] + 2)
            port_info_cell = ws.cell(row=slot_row, column=device_info['col'] + 3)
