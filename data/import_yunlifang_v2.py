        ...
        if device_cell.value and 'DC908' in str(device_cell.value):
            ...
            for slot_row in range(4, 12):
                ...
                if slot_number and str(slot_number).isdigit():
                    ...
                    # 创建板卡记录
                    cursor.execute('''
                        INSERT INTO cards (device_id, slot_number, card_type, port_info, wavelength, direction, status, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (device_id, slot_number, card_type, str(port_info) if port_info else None, 
                          wavelength, direction, status, datetime.now()))
                    cards_imported += 1
...
def show_summary():
    ...
    cursor.execute('''
        ...
        WHERE d.site_id = ? AND c.wavelength IS NOT NULL
        GROUP BY wavelength
    ...
    if wavelengths:
        print("波长列表: ", ", ".join([f"{w[0]}nm({w[1]})" for w in wavelengths[:15]]))
